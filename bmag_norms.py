import numpy as np
from .tools import get_windowed_filtered_bmag, correlation, HPF, gen_max_probe_range, get_bmag, get_windowed_bmag
...
def shot_bmag_refnorm(refPos, window, filt):
    ...
    for shot in range(0, 25):
        if filt == 0:
            wfBmag, time = get_windowed_bmag(
                pos=refPos, shot=shot, window=window)
        else:
            wfBmag, time = get_windowed_filtered_bmag(
                pos=refPos, shot=shot, window=window, filt=filt)

        norm_Mag, _ = correlation(wfBmag, wfBmag, time, mode='valid')
...
def shot_bmag_stdnorm(refPos, window, filt, max_probe_number=8):
    """ Outputs the correlation norm array w.r.t shot. The norm is product of 
    the standard deviations of the two windowed and filtered bmag signals. """

    maxRange = gen_max_probe_range(
    ...
    for posIndex, pos in enumerate(posArray):
        for shot in range(0, 25):
            ...
            Br_2, Bt_2, Bz_2, time = load_data(pos, shot)
            ...
            fBmag1 = HPF(Bmag1, filt)
            fBmag2 = HPF(Bmag2, filt)
...
def shot_bmag_refnorm_tof(refPos):
    ...
    for shot in range(0, 25):
        ...
        norm_Mag, _ = correlation(bmag, bmag, time, mode='valid')
...
def shot_bmag_stdnorm_tof(refPos, max_probe_number=8):
    ...
    for posIndex, pos in enumerate(posArray):
        for shot in range(0, 25):
            ...
            bmag, _ = get_bmag(pos=pos, shot=shot)
...
def shot_windowed_bmag_stdnorm(refPos, window, max_probe_number=8):
    ...
    for posIndex, pos in enumerate(posArray):
        for shot in range(0, 25):
            ...
            refBmag, _ = get_windowed_bmag(
                pos=refPos, shot=shot, window=window)
            bmag, _ = get_windowed_bmag(

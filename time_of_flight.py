...
from .bmag_spatial_correlation import gen_shot_pos_unfiltered_bmag_correlation
...
def compute_time_delay(refPos, pos, shot, window=[75, 150], norm='ref', normalized=False):
    ...
    zeroDex = finding_Index_Time(tau*1e-6, -10)
    limitDex = finding_Index_Time(tau*1e-6, 10)
...
def get_shot_correlation(refPos, pos, shot, window=[75, 150], norm='ref', normalized=False):
    ...
    specified probes.
    """
    magwCorr, tau = gen_shot_pos_unfiltered_bmag_correlation(

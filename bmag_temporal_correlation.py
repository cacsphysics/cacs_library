...
from .tools import get_windowed_filtered_bmag, correlation, get_pos_index
from .bmag_norms import which_norm
...
def gen_bmag_temporal_correlation(refPos, filt, window, norm='ref'):
    ...
    for shot in range(0, 25):
        Bmag1, time = get_windowed_filtered_bmag(
            refPos, shot, window, filt)
        corr_MagHold, tau = correlation(

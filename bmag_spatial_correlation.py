...
from .tools import gen_separation_array, get_windowed_filtered_bmag, correlation, HPF, get_pos_index, get_windowed_bmag, get_bmag
from .bmx_data import load_data
from .bmag_norms import which_norm, which_norm_tof, which_windowed_norm
...
def gen_bmag_spatial_correlation_plot(refPos, filt, window, norm='ref'):
    ...
    separation = gen_separation_array(refPos)
...
def gen_bmag_spatial_correlation(refPos, filt, window, norm='ref'):
    ...
    normArray = which_norm(norm, refPos, window, filt)
    ...
    for pos2Index, pos2 in enumerate([*range(refPos, 16, 2)]):
        for shot in range(0, 25):
            Bmag1, _ = get_windowed_filtered_bmag(
                refPos, shot, window, filt)
            Bmag2, time = get_windowed_filtered_bmag(
                pos2, shot, window, filt)
            corr_MagHold, _ = correlation(
...
def gen_shot_bmag_spatial_correlation(refPos, shot, filt=3e4, window=[75, 150], norm='ref'):
    ...
    for pos2Index, pos2 in enumerate([*range(refPos, 16, 2)]):
        Br_1, Bt_1, Bz_1, time = load_data(refPos, shot)
        Br_2, Bt_2, Bz_2, time = load_data(pos2, shot)
        ...
        if filt > 0:
            Bmag1 = HPF(Bmag1, filt)
            Bmag2 = HPF(Bmag2, filt)
        ...
        corrMagHold, _ = correlation(
            Bmag1, Bmag2, time, mode='valid')
        corrMagHold = corrMagHold
...
def gen_shot_pos_bmag_correlation(refPos, pos, shot, filt=3e4, window=[75, 150], norm='ref', normalized=True):
    ...
    bmagRef, time = get_windowed_filtered_bmag(
        pos=refPos, shot=shot, window=window, filt=filt)
    bmagPos, _ = get_windowed_filtered_bmag(pos, shot, window, filt)

    magCorr, tau = correlation(bmagRef, bmagPos, time)

    if normalized:
        posIndex = get_pos_index(refPos, pos)
...
def gen_shot_pos_unproccessed_bmag_correlation(refPos, pos, shot, norm='ref', normalized=True):
    ...
    """
    normArray = which_norm_tof(norm, refPos)
    ...
    def norm_value(norm, posIndex, shot):
        ...
        return normValue
    refBmag, _ = get_bmag(pos=refPos, shot=shot)
    bmag, time = get_bmag(pos=pos, shot=shot)

    magCorr, tau = correlation(refBmag, bmag, time)
...
def gen_shot_pos_unfiltered_bmag_correlation(refPos, pos, shot, window=[75, 150], norm='ref', normalized=False):
    ...
    normArray = which_windowed_norm(norm, refPos, window)
    ...
    def norm_value(norm, posIndex, shot):
        ...
        return normValue
    refBmag, _ = get_windowed_bmag(pos=refPos, shot=shot, window=window)
    bmag, time = get_windowed_bmag(pos=pos, shot=shot, window=window)
    # create new function and delete
    #refBmag -= np.mean(refBmag)
    #bmag -= np.mean(bmag)

    magCorr, tau = correlation(
...
def flatline_correction(abs_corr):
    """ Introducing a correlation correction to the absolute value.
    """
    abs_corr -= np.mean(abs_corr[-2:])
